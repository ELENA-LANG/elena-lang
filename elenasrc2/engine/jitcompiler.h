...
public:
   ...
   virtual void compileMssgExtension(MemoryWriter* writer, mssg_t low, int addr) = 0;
   ...
   virtual pos_t findMethodAddress(void* refVMT, mssg_t messageID, size_t vmtLength) = 0;
   virtual int findMethodIndex(void* refVMT, mssg_t messageID, size_t vmtLength) = 0;
   virtual size_t findFlags(void* refVMT) = 0;
   virtual size_t findLength(void* refVMT) = 0;
   virtual void* findClassPtr(void* refVMT) = 0;

   virtual void addVMTEntry(mssg_t message, size_t codePosition, VMTEntry* entries, size_t& count) = 0;
...
public:
   ...
   virtual void compileMssgExtension(MemoryWriter* writer, mssg_t low, int addr);
   ...
   virtual pos_t findMethodAddress(void* refVMT, mssg_t messageID, size_t vmtLength);
   virtual int findMethodIndex(void* refVMT, mssg_t messageID, size_t vmtLength);
...
public:
   ...
   virtual int findMethodIndex(void* refVMT, mssg_t messageID, size_t vmtLength);
   virtual int findMethodIndexX(void* refVMT, mssg64_t messageID, size_t vmtLength);
   virtual void* findClassPtr(void* refVMT);
   ...
   virtual void addVMTXEntry(mssg64_t message, size_t codePosition, VMTXEntry* entries, size_t& entryCount);
   virtual void fixVMT(MemoryWriter& vmtWriter, pos_t classClassVAddress, pos_t packageParentVAddress, int count, bool virtualMode);
