...
void Compiler :: declareVariable(SNode& terminal, CodeScope& scope, ref_t typeRef/*, bool dynamicArray, bool canBeIdle*/)
{
...
/*ref_t*/void Compiler :: compileMessageParameters(SNode node, ExprScope& scope, EAttr mode/*,
...
//            writer.newNode(lxExtArgument);
         ...
         // try to recognize the message signature
		   ObjectInfo paramInfo = compileExpression(current, scope, 
            mapObject(current, scope, paramMode), 0, paramMode);
...
//         // ignore duplicates
//      }
      /*else */declareVariable(current, *codeScope, typeRef/*, dynamicSize, !exprAttr.testany(HINT_REFOP)*/);
   }
...
//   //}
   ...
   ObjectInfo retVal = compileExpression(node, exprScope,
      mapObject(node, exprScope, rootMode), 0, rootMode);
...
////   }
////   else {
//      SNode callNode = node.prependSibling(lxCalling_1, scope.moduleScope->newobject_message);
...
{
   node.insertNode(lxAllocated, codeScope.level - preallocated);  // allocate the space for the local variables excluding preallocated ones ("$this", "$message")
   node.insertNode(lxArgCount, argCount/* + scope.rootToFree*/);
...
{
   bool implicitConstructor = false;
   if (test(scope.message, STATIC_MESSAGE)) {
      // check if it is implicit message
      ref_t dummyRef = 0;
      ident_t action = scope.module->resolveAction(getAction(scope.message), dummyRef);

      implicitConstructor = action.compare(CONSTRUCTOR_MESSAGE);
   }
...
//{
//   SNode methodNode = node.appendNode(lxConstructor, scope.message);
//
//   ClassScope* classScope = (ClassScope*)scope.getScope(Scope::ScopeLevel::slClass);
//
//   SNode op;
