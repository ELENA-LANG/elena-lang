...
inline SNode __fastcall findParent(SNode node, LexicalType type1, LexicalType type2)
{
...
void Compiler::NamespaceScope :: saveExtension(mssg_t message, ref_t extRef, ref_t strongMessage, bool internalOne)
{
...
//      SNode parentNode = roperandNode.parentNode();
      ...
      retVal = compileMessage(writer, node, scope, loperand, message, &arguments, 0/*, operationMode*/);
...
ObjectInfo Compiler :: compileBranchingOperation(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode, int operator_id)
{
   ...
   compileBranchingOp(writer, lnode.nextNode(lxObjectMask), scope, mode, operator_id, loperand, retVal);
...
//      bool dummy = false;
      retVal = compileMessage(writer, node, scope, loperand, messageRef, arguments, stackSafeAttr/*, operationMode*/);
   }
...
ObjectInfo Compiler :: compileOperation(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode, int operator_id)
{
   ...
   ObjectInfo roperand = compileExpression(writer, rnode, scope, 0, HINT_PARAMETER);
...
ObjectInfo Compiler :: compileOperationExpression(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode)
{
//   SNode current = node;
...
////      roperand = roperand.nextNode(lxObjectMask);
   ...
   switch (operator_id) {
      ...
      case IFNOT_OPERATOR_ID:
         ...
         return compileBranchingOperation(writer, node, scope, mode, operator_id);
...
void Compiler :: appendCopying(SyntaxWriter& writer, /*SNode& copyingNode, */int size/*, bool variadic, bool primArray*/, 
...
ObjectInfo Compiler :: boxArgumentInPlace(SyntaxWriter& writer, SNode node, ObjectInfo target/*, SNode objNode*/, ExprScope& scope,
...
//
         ...
         appendCopying(writer, /*copyingNode, */size/*, variadic, primArray*/, target, scope, tempLocal);
...
//   bool noUnboxing = EAttrs::test(mode, HINT_NOUNBOXING);
   ...
   if (!_logic->isCompatible(*scope.moduleScope, targetRef, sourceRef, false)) {
      ...
      //      }
         ...
         auto result = _logic->injectImplicitConversion(scope, node, *this, targetRef, sourceRef,
...
ObjectInfo Compiler :: compileAssigningExpression(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode)
{
   ...
   if (current == lxPropertyExpression) {
      ...
      ObjectInfo target = compileObject(writer, current, scope, mode | HINT_PARAMETER | HINT_NODEBUGINFO);
...
ObjectInfo Compiler :: compileMessageOperation(SyntaxWriter& writer, SNode current, ExprScope& scope, ObjectInfo target, mssg_t messageRef,
   ref_t expectedSignRef, EAttr paramsMode)
{
   ObjectInfo retVal;
   ...
   //      EAttr extMode = mode & HINT_ROOT;
      ...
      // NOTE : for external operation - expectedSignRef is a target type
      retVal = compileExternalCall(writer, current.parentNode(), scope, expectedSignRef, &arguments);
   }
   else {
   //      if (variadicOne)
   ...
   //      else {
            ...
            retVal = compileMessage(writer, current, scope, target, messageRef/*, mode, withEmbeddableRet*/, &arguments, stackSafeAttr);
   //
...
ObjectInfo Compiler :: compileMessageExpression(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode)
{
   SNode current = node.firstChild();
   ...
   EAttrs objMode(mode | HINT_TARGET, HINT_PROP_MODE);
   ObjectInfo target = compileObject(writer, current, scope, objMode);
   ...
   if (target.kind == okNewOp) {
      ...
      // HOTFIX : for external operation - saving of the arguments happends later
      else isParam = false;
   }
...
ObjectInfo Compiler :: compileArrAssigning(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode)
{
   ...
   ObjectInfo roperand = compileExpression(writer, rnode, scope, 0, HINT_PARAMETER);
   ObjectInfo roperand2 = compileExpression(writer, rnode2, scope, 0, HINT_PARAMETER);
...
ObjectInfo Compiler :: compilePropAssigning(SyntaxWriter& writer, SNode node, ExprScope& scope, EAttr mode)
{
   ...
   ObjectInfo target = compileObject(writer, current.firstChild(), scope, objMode);
...
//   }
   ...
   if (multiMethod) {
      ...
      _logic->injectVirtualMultimethods(*scope.moduleScope, node, *this, implicitMultimethods, lxClassMethod, scope.info);
...
ObjectInfo Compiler :: compileClosure(SyntaxWriter& writer, SNode node, ExprScope& ownerScope, InlineClassScope& scope, EAttr mode)
{
//   bool noUnboxing = EAttrs::test(mode, HINT_NOUNBOXING);
...
ObjectInfo Compiler :: compileClosure(SyntaxWriter& writer, SNode node, ExprScope& ownerScope, EAttr mode)
{
   ref_t nestedRef = 0;
...
//   else compileNestedVMT(node, scope);
   ...
   return compileClosure(writer, node, ownerScope, scope, mode);
}
...
ObjectInfo Compiler :: compileCastingExpression(SyntaxWriter& writer, SNode node, ExprScope& scope, ObjectInfo target, EAttr mode)
{
   ...
   int paramCount = SyntaxTree::countNodeMask(node.parentNode(), lxObjectMask);
   if (paramCount == 1) {
      // if it is a cast expression
      writeTerminal(writer, target, scope);

      SNode opNode = node.parentNode();
      retVal = convertObject(writer, opNode, scope, targetRef, target, mode);
...
//   //   EAttrs mode(modeAttr);
   ...
   {
      bool invalid = false;
      if (EAttrs::test(mode, HINT_NEWOP)) {
         if (EAttrs::test(mode, HINT_TARGET)) {
            ...
            else object = scope.mapTerminal(token, node == lxReference, EAttrs::exclude(mode, HINT_NEWOP));
            //   ////      if (current == lxArrayType) {
      ...
      else if (EAttrs::test(mode, HINT_CASTOP)) {
         if (node == lxType) {
            ref_t typeRef = resolveTypeAttribute(node, scope, false, false);
            object = mapClassSymbol(scope, typeRef);

            if (object.kind == okClass) {
               object.kind = okCastOp;
            }
            else invalid = false;
         }
         else invalid = false;
...
//   return result;
   ...
   else return compileExpression(writer, node, scope, 0, mode);
}

ObjectInfo Compiler :: compileExpression(SyntaxWriter& writer, SNode node, ExprScope& scope, ref_t targetRef, EAttr mode)
{
   bool noPrimMode = /*EAttrs::test(modeAttrs, HINT_NOPRIMITIVES)*/false;
   ...
   switch (node.type) {
      case lxExpression:
         retVal = compileExpression(writer, node.firstChild(), scope, targetRef, mode);
         targetRef = 0; // the result should be typecasted in the sub call
         break;
...
//ObjectInfo Compiler :: compileInternalCall(SNode node, ExprScope& scope, mssg_t message, ref_t signature, ObjectInfo routine)
//{
//   _ModuleScope* moduleScope = scope.moduleScope;
//
...
void Compiler :: compileDispatchExpression(SyntaxWriter& writer, SNode node, CodeScope& scope, bool withGenericMethods)
{
   ...
   if (target.kind == okInternal) {
      ...
      if (methodScope->message == methodScope->moduleScope->dispatch_message) {
         ...
         target = compileExpression(writer, node.firstChild(), exprScope, 0, EAttr::eaNone);

         if (boxingRequired(target)) {
            ObjectInfo boxedArg = boxArgumentInPlace(writer, node, target, exprScope, condBoxingRequired(target));

            writeTerminal(writer, boxedArg, exprScope);
         }
         ...
         ObjectInfo retVal = compileMessage(writer, exprNode, exprScope, target, methodScope->message, &arguments, 
            /*mode | HINT_NODEBUGINFO, */stackSafeAttrs/*, dummy*/);
...
inline SNode __fastcall findInitNode(SNode node, ident_t name)
{
   SNode current = node.firstChild();
   while (current != lxNone) {
      if (current == lxFieldInit) {
...
inline SNode __fastcall findName(SNode node)
{
...
inline bool __fastcall checkNonpublicDuplicates(ClassInfo& info, mssg_t publicMessage)
{
   for (auto it = info.methodHints.start(); !it.Eof(); it++) {
      Attribute key = it.key();
...
void Compiler :: compileSymbolImplementation(SyntaxTree& expressionTree, SNode node, SymbolScope& scope)
{
   ...
   ObjectInfo retVal = compileExpression(writer, expression, exprScope,
      /*mapObject(expression, exprScope, HINT_ROOTSYMBOL),*/ 0, exprMode | HINT_ROOTSYMBOL);
...
void Compiler :: compileStaticAssigning(SNode node, ClassScope& scope/*, int mode*/)
{
   ...
   ObjectInfo source = compileExpression(writer, node, exprScope, /*target.extraparam*/0, HINT_NODEBUGINFO);
...
//void Compiler :: injectVirtualDispatchMethod(SNode classNode, mssg_t message, LexicalType type, ident_t argument)
//{
//   SyntaxTree subTree;
