...
//}
//
//void Compiler :: importCode(SNode node, Scope& scope, ref_t functionRef, mssg_t message)
//{
//   _ModuleScope* moduleScope = scope.moduleScope;
//
...
void Compiler :: declareVariable(SyntaxWriter& writer, SNode terminal, ExprScope& scope, ref_t typeRef/*, bool dynamicArray*/, bool canBeIdle)
{
...
//   }
   ...
   if (!codeScope->locals.exist(identifier)) {
      ...
      SNode varNode = rootNode.insertNode(variableType, variableArg);
...
mssg_t Compiler :: mapMessage(SNode node, ExprScope& scope, /*bool extensionCall, */ bool newOpCall)
{
   ref_t actionFlags = 0;
//   if (extensionCall)
...
//}
//
//ref_t Compiler :: resolveOperatorMessage(Scope& scope, ref_t operator_id, size_t argCount)
//{
//   switch (operator_id) {
//      case IF_OPERATOR_ID:
...
//   scope.originals.clear();
   ...
   writeTerminal(writer, target);
   if (arguments != nullptr) {
      for (unsigned int i = 0; i < arguments->Length(); i++) {
         writeTerminal(writer, (*arguments)[i]);
      }
   }
...
ObjectInfo Compiler :: convertObject(SyntaxWriter& writer, ExprScope& scope, ref_t targetRef, ObjectInfo source, EAttr mode)
{
...
//   int stackSafeAttrs = 0;
   if (!_logic->isCompatible(*scope.moduleScope, targetRef, sourceRef, false)) {
      ...
      //      }
         ...
         //}
         /*else*/ return sendTypecast(writer, scope, targetRef, source);
      }
...
ObjectInfo Compiler :: sendTypecast(SyntaxWriter& writer, ExprScope& scope, ref_t targetRef, ObjectInfo source)
{
   if (targetRef != 0 /*&& !isPrimitiveRef(targetRef)*/) {
      if (targetRef != scope.moduleScope->superReference) {
         ...
         bool dummy;
         compileMessage(writer, scope, source, encodeMessage(actionRef, 1, CONVERSION_MESSAGE), nullptr/*
...
{
   SNode current = node.firstChild();
   ObjectInfo target = compileObject(writer, current, scope, mode | HINT_PARAMETER);
...
//   ref_t expectedSignRef = 0; // contains the expected message signature if it there is only single method overloading
   ...
   if (target.kind == okNewOp) {
      messageRef = mapMessage(node, scope/*, false*/, true);
      if (getAction(messageRef) == getAction(scope.moduleScope->constructor_message)) {
         target.kind = okClass;
      }
      else scope.raiseError(errInvalidOperation, node);
   }
   else /*if (target.kind != okExternal)*/ {
      messageRef = mapMessage(node, scope/*, target.kind == okExtension*/, false);
...
//         bool withEmbeddableRet = false;
         retVal = compileMessage(writer, /*opNode, */scope, target, messageRef/*, mode, stackSafeAttr, withEmbeddableRet*/, &arguments);
//
...
//   mssg_t messageRef = mapMessage(node, scope, false);
//   ref_t actionRef, flags;
//   size_t argCount;
//   decodeMessage(messageRef, actionRef, argCount, flags);
...
//   node.injectNode(lxExpression);
   ExprScope exprScope(&scope);
   ObjectInfo info = compileExpression(writer, node, exprScope, targetRef, mode);
...
EAttr Compiler :: declareExpressionAttributes(SyntaxWriter& writer, SNode& current, ExprScope& scope, EAttr)
{
   ...
   if (newVariable) {
      ...
      else declareVariable(writer, current, scope, typeRef/*, dynamicSize*/, /*!exprAttr.testany(HINT_REFOP)*/true);
   }
...
//   //   EAttrs mode(modeAttr);
   ...
   if (EAttrs::testany(mode, /*HINT_INTERNALOP | HINT_MEMBER | HINT_METAFIELD | HINT_EXTERNALOP | HINT_FORWARD
      | HINT_MESSAGEREF | HINT_SUBJECTREF*/HINT_NEWOP))
   {
      bool invalid = false;
      if (EAttrs::test(mode, HINT_NEWOP)) {
         if (EAttrs::test(mode, HINT_TARGET)) {
            object = scope.mapTerminal(token, node == lxReference, EAttrs::exclude(mode, HINT_NEWOP));
            //   ////      if (current == lxArrayType) {
...
void Compiler :: writeTerminal(SyntaxWriter& writer, ObjectInfo object)
{
   switch (object.kind) {
      case okNil:
...
//   }
   ...
   if (targetRef) {
      ...
      retVal = convertObject(writer, scope, targetRef, retVal, mode);
      if (retVal.kind == okUnknown) {
         scope.raiseError(errInvalidOperation, node);
      }
   }
...
//      compileMessage(messageNode, scope, 0, target, resendMode);
//
...
void Compiler :: compileMethodCode(SyntaxWriter& writer, SNode body, MethodScope& scope, CodeScope& codeScope)
{
...
//   }
   ...
   if (retVal.kind == okUnknown && !codeScope.withRetStatement) {
      ...
      if (resultRef != 0) {
         if (convertObject(writer, exprScope, resultRef, retVal, EAttr::eaNone).kind == okUnknown)
            scope.raiseError(errInvalidOperation, body.findChild(lxEOP));
      }
      writer.closeNode();
   }
...
//
//   SNode resendExpr = resendNode.appendNode(lxExpression);
...
//
//      SNode exprNode = resendExpr.appendNode(lxExpression);
//      if (signatures[i] != moduleScope.superReference) {
