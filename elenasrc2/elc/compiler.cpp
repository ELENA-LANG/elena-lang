...
void Compiler :: compileVMT(SyntaxWriter& writer, SNode node, ClassScope& scope, bool ignoreAutogenerated)
{
   SNode current = node.firstChild();
   ...
   while (current != lxNone) {
      switch(current) {
         case lxClassMethod:
         {
            if (ignoreAutogenerated && current.existChild(lxAutogenerated)) {
               current = current.nextNode();
               continue;
            }               
