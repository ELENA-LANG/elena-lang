...
ref_t Compiler :: mapMessage(SNode node, CodeScope& scope, size_t& paramCount)
{
   ...
   if (verb_id == 0) {
      ref_t verbRef = scope.mapSubject(name, signature);
      if (verbRef != 0) {
         if (arg.nextNode() == lxNone) {
            signature.append('$');
            signature.append(scope.moduleScope->module->resolveReference(verbRef));
         }
         else scope.raiseError(errStrongTypeNotAllowed, arg);
      }
      first = false;
   }
   ...
   while (arg == lxMessage) {
      ...
      ref_t classRef = scope.mapSubject(subject, signature);
      if (classRef) {
...
bool Compiler :: typecastObject(SyntaxWriter& writer, ModuleScope& scope, ref_t targetRef)
{
   if (targetRef != 0) {
      ...
      writer.insert(lxCalling, encodeMessage(scope.module->mapSubject(sign, false), GET_MESSAGE_ID, 0));
      writer.closeNode();
...
void Compiler :: declareArgumentList(SNode node, MethodScope& scope)
{
   ...
   while (arg == lxMessage || arg == lxParamRefAttr) {
      ...
      if (arg == lxMessage && subject != lxReference) {
         ...
         class_ref = scope.mapSubject(subject, signature);
         if (class_ref) {
            size_t len = signature.Length();
            if (len == 0) {
               signature.append('$');
...
//      }
      ...
      if (overloadList && paramCount > 0 && verb_id != PRIVATE_MESSAGE_ID) {
...
void Compiler :: compileConstructor(SyntaxWriter& writer, SNode node, MethodScope& scope, ClassScope& classClassScope)
{
   ...
   if (bodyNode == lxDispatchCode) {
      ...
      if (scope.multiMethod && bodyNode.argument != 0) {
         ref_t overloadRef = classClassScope.info.methodHints.get(Attribute(scope.message, maOverloadlist));
         if (overloadRef) {
            writer.appendNode(lxMultiDispatching, overloadRef);
         }
         else scope.raiseError(errIllegalOperation, node.parentNode());
...
void Compiler :: generateMethodDeclaration(SNode current, ClassScope& scope, bool hideDuplicates, bool closed)
{
   ...
   if (scope.info.methods.exist(message, true) && hideDuplicates) {
      ...
      while (prev != lxNone) {
         if (prev == lxClassMethod && prev.argument == message && prev.existChild(lxTemplate))
...
void Compiler :: defineEmbeddableAttributes(ClassScope& classScope, SNode methodNode)
{
   ...
   if (_logic->recognizeEmbeddableGet(*classScope.moduleScope, methodNode, classScope.extensionMode != 0 ? classScope.reference : 0, returnRef, type)) {
   ...
   else if (_logic->recognizeEmbeddableGetAt(*classScope.moduleScope, methodNode, classScope.extensionMode != 0 ? classScope.reference : 0, returnRef, type)) {
   ...
   // Optimization : var = getAt&int => read&int&subject&var[2]
   else if (_logic->recognizeEmbeddableGetAt2(*classScope.moduleScope, methodNode, classScope.extensionMode != 0 ? classScope.reference : 0, returnRef, type)) {
   ...
   else if (_logic->recognizeEmbeddableEval(*classScope.moduleScope, methodNode, classScope.extensionMode != 0 ? classScope.reference : 0, returnRef, type)) {
   ...
   else if (_logic->recognizeEmbeddableEval2(*classScope.moduleScope, methodNode, classScope.extensionMode != 0 ? classScope.reference : 0, returnRef, type)) {
...
void Compiler :: injectVirtualMultimethod(_CompilerScope& scope, SNode classNode, ref_t message, LexicalType methodType, ref_t parentRef)
{
   ...
   if (!parentRef) {
      ...
      resendMessage = encodeMessage(signRef, getVerb(message), paramCount);
   }
