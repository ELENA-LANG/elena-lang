   ...
   while (!it.Eof() && level > 0) {
      ...
      switch (*it) {
         ...
         case bdSelf:
            ...
            break;
         case bdLocal:
            writeLocal(scope, (const char*)(*it).Argument(), (*it).additional, frameLevel);
            break;
         case bdIntLocal:
...
//         tape.write(bcDLoadSI, 0);
//         tape.write(bcAddN, 4);
//         tape.write(bcLess, baCurrentLabel);
//         tape.releaseLabel();
//         tape.write(bcPopI, 2);
//      }
//   }
...
{
   SNode terminal = (node.findChild(lxRow) == lxRow) ? node : node.findNext(lxObjectMask);
...
//               SyntaxTree::findChild(current, lxTerminal).identifier(),
//               level, SyntaxTree::findChild(current, lxClassName).identifier());
//            break;
//         }
//         case lxReleasing:
//            releaseObject(tape, current.argument);
