...
void DerivationTransformer :: autoGenerateExtensions(DerivationScope& templateScope)
{
   ...
   if (list) {
      for (auto it = list->start(); !it.Eof(); it++) {
         DerivationScope scope(templateScope.compilerScope, NULL, NULL, NULL);
         scope.templateRef = *it;

         // copy parameters
         for (auto param_it = templateScope.parameters.start(); !param_it.Eof(); param_it++) {
            scope.parameterValues.add(templateScope.parameterValues.Count() + 1, *param_it);
         }
...
ref_t DerivationTransformer :: generateNewTemplate(DerivationScope& templateScope)
{
   ...
   for (auto it = templateScope.parameters.start(); !it.Eof(); it++) {
      parameters.add(*it);
   }
