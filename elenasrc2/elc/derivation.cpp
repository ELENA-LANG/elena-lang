...
void DerivationTransformer :: declareAttribute(SNode node, DerivationScope& scope)
{
   ...
   if (scope.compilerScope->attributes.add(name, node.argument, true)) {
...
void DerivationTransformer :: generate(SyntaxWriter& writer, SNode node, CompilerScope& scope, ident_t sourcePath, ident_t ns,
...
void DerivationTransformer :: generate(SyntaxWriter& writer, CompilerScope& scope, ident_t sourcePath, ident_t ns, IdentifierList* imports)
{
...
void DerivationTransformer :: recognize(CompilerScope& scope, SNode node, ident_t sourcePath, ident_t ns)
{
   SNode current = node.firstChild();
   while (current != lxNone) {
      if (current == lxScope) {
...
void DerivationTransformer :: recognize(CompilerScope& scope, ident_t sourcePath, ident_t ns)
{
