...
void DerivationReader :: generateNewTemplate(SyntaxWriter& writer, SNode current, DerivationScope& scope, bool templateMode)
{
   ...
   int prefixCounter = SyntaxTree::countChild(operatorNode, lxObject, lxExpression);

   attrRef = scope.mapAttribute(attr/*, true*/);
   //if ( == V_TYPETEMPL && prefixCounter == 1) {
   //   attrRef = V_TYPETEMPL;
   ...
   if (operatorNode.existChild(lxSize) && prefixCounter == 1 && !isPrimitiveRef(attrRef)) {
...
bool DerivationReader :: checkTemplateExpression(SNode current, DerivationScope& scope)
{
   ...
   SNode nextNode = current.nextNode();
   if (nextNode == lxOperator && nextNode.existChild(lxSize)) {
