...
//
         ref_t classRef = scope.mapIdentifier(terminalNode.identifier());
...
//   }
   /*else */if (classMode) {
      typeRef = generateNewTemplate(scope, attrRef, operatorNode.findChild(lxAttributeValue));
   }
//   else if (arrayMode) {
//      typeRef = attrRef;
//   }
...
//      SNode paramNode = current.nextNode();
      generateTemplateParameters(current.prevNode(), scope, scope.reference == INVALID_REF);
//
...
//   }
   /*else */if (attrRef == V_TYPETEMPL/* || attrRef == V_OBJARRAY*/) {
      classRef = scope.mapIdentifier(attr.firstChild(lxTerminalMask).identifier());
...
bool DerivationTransformer ::recognizeDeclaration(SNode node, DerivationScope& scope/*, SNode attributes*/)
{
...
//      }
      ...
      node.set(lxTemplate, scope.compilerScope->mapNewIdentifier(templateName.c_str(), false));
...
{
   // try to recognize general declaration
   if (!recognizeDeclaration(node, scope/*, attributes*/)) {
      //         attributes.refresh();
      ...
      else if (node.compare(lxClass, lxSymbol)) {
