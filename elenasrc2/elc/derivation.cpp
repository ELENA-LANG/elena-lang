...
ref_t DerivationReader :: generateTemplate(SNode current, DerivationScope& scope, SubjectMap* parentAttributes)
{
   ...
   generateTemplate(bufferWriter, templateScope, true, false);
...
void DerivationReader :: copyTemplateTree(SyntaxWriter& writer, SNode node, DerivationScope& scope, SNode attributeValues, SubjectMap* parentAttributes)
{
   ...
   if (generateTemplate(writer, scope, false, false)) {
   ...
   while (current != lxNone) {
      ...
      /*else */if (current == lxAttributeValue || current == lxIdleAttribute) {
         ...
         if (attrNode.existChild(lxAttributeValue)) {
            generateAttributeTemplate(writer, attrNode, scope, true, false);
         }
         else {
            int paramIndex = 0;
...
bool DerivationReader :: generateTemplate(SyntaxWriter& writer, DerivationScope& scope, bool declaringClass, bool baseClass)
{
   ...
   if (declaringClass) {
      ...
      else writer.appendNode(lxAttribute, V_ABSTRACT);
   }
...
void DerivationReader :: generateMessage(SyntaxWriter& writer, SNode current, DerivationScope& scope, bool templateMode)
{
   ...
   else generateAttributeTemplate(writer, current, scope, templateMode, false);
   writer.closeNode();
...
void DerivationReader :: generateAttributeTemplate(SyntaxWriter& writer, SNode node, DerivationScope& scope, bool templateMode, bool baseClassMode)
{
   IdentifierString attrName;
   ...
   if (templateMode) {
      ...
      SyntaxTree buffer;
      SyntaxWriter bufferWriter(buffer);
      generateTemplate(bufferWriter, templateScope, true, baseClassMode);
...
bool DerivationReader :: declareType(SyntaxWriter& writer, SNode node, DerivationScope& scope, SNode attributes)
{
   ...
   if (classNode != lxNone) {
      if (templateMode) {
         ...
         if (generateTemplate(writer, templateScope, true, false)) {
   ...
   while (current != lxNone) {
      if (current == lxAttribute || current == lxNameAttr) {
         ...
         if (current.existChild(lxAttributeValue)) {
            if (firstParent) {               
               writer.newNode(lxBaseParent);
               generateAttributeTemplate(writer, current, scope, false, true);
               writer.closeNode();
            }
            else {
...
void DerivationReader :: generateSyntaxTree(SyntaxWriter& writer, _CompilerScope& scope)
{
   ...
   SyntaxWriter bufferWriter(generatedTree);
   generateSyntaxTree(bufferWriter, _root, scope, autogeneratedTree);

   writer.newNode(lxRoot);
   SyntaxTree::moveNodes(writer, autogeneratedTree, lxClass);
   SyntaxTree::moveNodes(writer, generatedTree, lxClass, lxSymbol, lxInclude);
   writer.closeNode();
}
