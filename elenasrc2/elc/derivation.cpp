...
void DerivationReader :: generateNewTemplate(SyntaxWriter& writer, SNode current, DerivationScope& scope, bool templateMode)
{
   ...
   int prefixCounter = SyntaxTree::countNode(operatorNode, lxOperator) - 1;
...
bool DerivationReader :: checkTemplateExpression(SNode current, DerivationScope&)
{
   ...
   if (nextNode == lxOperator) {
      ...
      else if (nextNode.findChild(lxTerminal).identifier().compare("<") && nextNode.existChild(lxAngleOperator)) {
         return true;
      }
   }
...
inline bool checkVarTemplateBrackets(SNode node, SNode& current)
{
   if (node.firstChild().findChild(lxTerminal).identifier().compare("<")) {
      current = node.nextNode();
      while (current != lxNone) {
   ...
   if (node.existChild(lxAssigning)) {
      ...
      if (nextNode == lxMessage) {
         ...
         if (attrRef != 0) {
            ...
            SNode operatorNode;
            if (checkVarTemplateBrackets(nextNode, operatorNode)) {
               // make tree transformation
               current.set(lxAttribute, -1);
               ...
               setTemplateAttributes(nextNode);
