...
ref_t DerivationTransformer::DerivationScope :: mapAttribute(SNode attribute)
{
...
//   reference = compilerScope->mapTemplateClass(name, alreadyDeclared);
//
...
//      writer.appendNode(lxReference, scope.compilerScope->module->resolveReference(scope.reference));
//   }
//
//   //SyntaxTree buffer;
//
...
ref_t DerivationTransformer :: mapAttribute(SNode terminal, DerivationScope& scope/*, bool& templateAttr*/)
{
   //      int paramIndex = 0;
...
void DerivationTransformer :: generateAttributes(SyntaxWriter& writer, SNode node, DerivationScope& scope, bool symbolMode/*, bool templateMode*/)
{
   SNode current = node.prevNode();
...
//      bool paramMode = false;
      ...
      ref_t ref = scope.mapIdentifier(typeNode.identifier());
      if (ref) {
      ...
      copyIdentifier(writer, typeNode);
...
//
//
//void DerivationTransformer :: generateTemplateParameters(SNode node, DerivationScope& scope/*, bool templateMode*/)
//{
//   SNode current = node.prevNode();
//   if (isClosingOperator(current)) {
//      // HOTFIX : remove operator terminal
//      SNode objectNode = current.firstChild();
//      objectNode = lxIdle;
//
//      objectNode = current.findChild(lxObject);
//      if (objectNode != lxNone) {
//         current = lxAttribute;
//
//         SNode terminalNode = objectNode.firstChild(lxTerminalMask);
//         objectNode.set(terminalNode.type, terminalNode.identifier());
//      }
//
//      current = current.prevNode();
//   }
...
//
//   if (!isClosingOperator(current)) {
//      SNode attrNode = current.prevNode();
//      if (attrNode == lxObject) {
//         current.set(lxAttribute, attrNode.firstChild(lxTerminalMask).identifier());
//         attrNode = lxIdle;
//         // NOTE : skip the operator terminal
//         SNode paramNode = current.firstChild().nextNode();
//         while (paramNode != lxNone) {
//            if (paramNode == lxObject) {
//               paramNode = lxAttributeValue;
//            }
...
//   //SNode current = node.prevNode();
//   //// modify closing 
//   //if (isClosingOperator(current) && current.existChild(lxObject)) {
//   //   // simulate attribute layout
//   //   modifyTemplateOperand(current);
//
//   //   current = current.prevNode();
//   //}
//
...
//   //   if (current == lxOperator) {
//   //      /*if (isClosingOperator(current)) {
//   //         generateSubTemplate(current, scope, templateMode);
//   //      }*/
//   //      // else {
//   //      modifyTemplateOperand(current);
//   //      current = current.prevNode();
...
//   //   current = lxAttribute;
//   //}
//   //else scope.raiseError(errInvalidSyntax, lastNode);
//}
//
...
//      generateTemplateParameters(current, scope/*, scope.reference == INVALID_REF*/);
////
...
////}
//
//void DerivationTransformer :: generateAttributeTemplate(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, bool templateMode, int mode*/)
//{
//    IdentifierString className; // either className or classRef should be set
//    ref_t classRef = 0;
//
////   bool  arrayMode = false;
...
//
//      className.copy(identNode.identifier());
//      className.append('#');
//      className.appendInt(prefixCounter);
//
//      attrRef = scope.mapIdentifier(className.c_str());
//      if (!attrRef)
...
//   }
//
//   SNode attr = node.findChild(lxAttributeValue);
////   if (node.argument == INVALID_REF) {
...
//      writer.newNode(lxClassRefAttr, className.c_str());
//   }
//   else if (classRef) {
...
void DerivationTransformer :: generateCodeTree(SyntaxWriter& writer, SNode node, DerivationScope& scope)
{
   writer.newNode(node.type, node.argument);
...
/*bool*/void DerivationTransformer :: generateFieldTree(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, SNode attributes, SyntaxTree& buffer, bool templateMode*/)
{
   if (node == lxClassField) {
      ...
      //}
      /*else */generateAttributes(writer, node, scope, false/*, templateMode*/);
...
void DerivationTransformer :: generateMethodTree(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, bool templateMode*/)
{
   ...
   generateAttributes(writer, node, scope, false/*, templateMode*/);
...
////}
//
//bool DerivationTransformer :: generateDeclaration(SNode node, DerivationScope& scope/*, SNode attributes*/)
//{
...
//
//      ref_t templateRef = scope.compilerScope->mapNewIdentifier(templateName.c_str(), false);
//
...
//
//      saveTemplate(node, scope, /*attributes, */scope.type, /**scope.autogeneratedTree, */templateRef);
//
...
//
//   generateAttributes(writer, node, scope, false/*, true*/);
//
//   SNode current = node.firstChild();
////   SNode subAttributes;
//   while (current != lxNone) {
...
void DerivationTransformer :: generateClassTree(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, SNode attributes, *int nested*/)
{
//   SyntaxTree buffer((pos_t)0);
//
//   if (!nested) {
      ...
      generateAttributes(writer, node, scope, true);
//      if (node.argument == -2) {
...
//}
//
//void DerivationTransformer :: saveTemplate(SNode node, DerivationScope& scope, /*SNode attributes, */DerivationScope::Type type, /*SyntaxTree& autogenerated, */ref_t templateRef)
//{
...
void DerivationTransformer :: generate(SyntaxWriter& writer, SNode node, CompilerScope& scope, ident_t sourcePath, MessageMap* attributes, IdentifierList* imports)
{
   ////   SNode attributes;
   SNode current = node.firstChild();
   while (current != lxNone) {
...
void DerivationTransformer :: generate(SyntaxWriter& writer, CompilerScope& scope, MessageMap* attributes)
{
   ...
   while (current != lxNone) {
      if (current == lxNamespace) {
         ...
         generate(writer, current.firstChild(), scope, current.identifier(), attributes, &imports);
...
void DerivationTransformer :: recognizeAttributes(SNode node, DerivationScope& scope)
{
   SNode current = node.prevNode();
   ...
   //      }
      ...
      nameNode.setArgument(scope.compilerScope->mapNewTerminal(nameNode.firstChild(lxTerminalMask), privateOne));

      //   if (withDeclaration)
      //      writer.appendNode(lxNameAttr, compilerScope->mapNewTerminal(terminal, privateOne));
      //

   //   if (nameNode != lxNone) {
   //      if (symbolMode) {
   //         scope.copyName(writer, nameNode, privateOne, true);
   //      }
   //      else scope.copyMessageName(writer, nameNode);
   //   }
}

void DerivationTransformer :: recognizeScope(SNode node, DerivationScope& scope, int mode)
{
   //      // try to recognize general declaration
   //      if (!generateDeclaration(node, scope/*, attributes*/)) {
   ////         attributes.refresh();
   ...
   /*else */if (node == lxAttributeDecl) {
      ...
      if (node.compare(lxClass, lxSymbol)) {
         recognizeAttributes(node, scope);
      }
      //         if (node == lxSymbol) {
      ////            if (!generateSingletonScope(writer, node, scope, attributes)) {
      //               generateSymbolTree(writer, node, scope/*, attributes*/);
...
void DerivationTransformer :: recognize(CompilerScope& scope, SNode node, ident_t sourcePath, MessageMap* attributes)
{
   ...
   while (current != lxNone) {
      ...
      //         //            rootScope.autogeneratedTree = &autogenerated;
         setIdentifier(current);
      //         generateScope(writer, current, rootScope/*, attributes*/, MODE_ROOT);
         recognizeScope(current, rootScope, MODE_ROOT);
      }
      current = current.nextNode();
   }
}
...
void DerivationTransformer :: recognize(CompilerScope& scope, MessageMap* attributes)
{
   ...
   //   generateSyntaxTree(writer, _root, scope, autogeneratedTree, &attributes, &imports, sourcePath);
   SNode current = _root.firstChild();
   while (current != lxNone) {
      if (current == lxNamespace) {
