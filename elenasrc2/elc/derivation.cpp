...
void DerivationTransformer :: generateAttributes(SyntaxWriter& writer, SNode node, DerivationScope& scope, bool rootMode/*, bool templateMode*/)
{
   SNode current = node.prevNode();
...
//      ref_t ref = mapAttribute(typeNode, scope, /*arrayMode, */paramMode);
//      if (paramMode) {
...
//
//      recognizeScopeMembers(node, scope, 0);
...
void DerivationTransformer :: recognizeRootAttributes(SNode node, DerivationScope& scope)
{
   ...
   ident_t name = nameNode.firstChild(lxTerminalMask).identifier();
