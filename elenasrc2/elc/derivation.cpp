   ...
   while (current != lxNone) {
      ...
      //      subject = moduleScope->module->mapSubject(identifier, false);
         ...
         if (item != lxNone && item.nextNode() == lxNone) {
            ref_t attr = 0;
            if (classMode) {
               attr = mapTerminal(item.findChild(lxIdentifier));
            }
...
void DerivationReader :: copyAttributeTree(SyntaxWriter& writer, SNode node, DerivationScope& scope)
{
   // copy coordinate
...
void DerivationReader :: generateTemplateVariableTree(SyntaxWriter& writer, SNode node, DerivationScope& scope, bool templateMode)
{
   ...
   ref_t attrRef = /*classMode ? */scope.mapClassTemplate(node.findChild(lxAttribute))/* : scope.mapTemplate(attrName)*/;
   if (!attrRef)
