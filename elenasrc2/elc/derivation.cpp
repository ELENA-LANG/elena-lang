...
void DerivationTransformer::DerivationScope :: copyName(SyntaxWriter& writer, SNode terminal, bool privateOne, bool withDeclaration)
{
   if (withDeclaration)
...
//}
//
//bool DerivationReader :: generateTemplate(SyntaxWriter& writer, DerivationScope& scope, bool declaringClass, int mode)
//{
...
void DerivationTransformer :: generateAttributes(SyntaxWriter& writer, SNode node, DerivationScope& scope, bool symbolMode/*, bool templateMode*/)
{
   ...
   if (current == lxNameAttr) {
      nameNode = current.firstChild(lxTerminalMask);
...
//}
//
//ref_t DerivationReader :: generateNewTemplate(DerivationScope& scope, ref_t attrRef, SNode node)
//{
//   DerivationScope templateScope(&scope, attrRef);
//   loadAttributeValues(node, templateScope, &scope.attributes, true);
//
//   SyntaxTree buffer;
//   SyntaxWriter bufferWriter(buffer);
//   generateTemplate(bufferWriter, templateScope, true);
...
//         case lxReference:
            writer.newNode(lxMessage);
            scope.copyMessageName(writer, current);
            writer.closeNode();
            break;
//         case lxOperator:
//         case lxObject:
...
void DerivationTransformer :: generateSymbolTree(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, SNode attributes*/)
{
   ...
   generateAttributes(writer, node, scope, true/*, attributes, false*/);
...
//      attrName.appendInt(prefixCounter);
//
      attrRef = /*scope.moduleScope->attributes.get(attrName)*/0; // !! temporal
...
//   }
   /*else */if (attrRef == V_TYPETEMPL/* || attrRef == V_OBJARRAY*/) {
      attrName.copy(attr.firstChild(lxTerminalMask).identifier());
   }
//   else {
//      typeRef = generateNewTemplate(scope, attrRef, attr);
//   }
...
//   if (node == lxAttribute) {
      writer.newNode(lxClassRefAttr, /*scope.moduleScope->module->resolveReference(typeRef)*/attrName.c_str());
//   }
...
/*bool*/void DerivationTransformer :: generateFieldTree(SyntaxWriter& writer, SNode node, DerivationScope& scope/*, SNode attributes, SyntaxTree& buffer, bool templateMode*/)
{
   if (node == lxClassField) {
      ...
      //}
      /*else */generateAttributes(writer, node, scope, false/*, attributes, templateMode*/);
...
//
//      saveTemplate(node, *scope.moduleScope, attributes, scope.type, *scope.autogeneratedTree, templateRef);
//
...
void DerivationTransformer :: generateScopeMembers(SNode& node, DerivationScope& scope, int mode)
{
   SNode current = node.firstChild();
   while (current != lxNone) {
      if (current == lxAttribute) {
         ...
         if (!generateMethodScope(current, scope)) {
...
void DerivationTransformer :: generateSyntaxTree(SyntaxWriter& writer, SNode node, CompilerScope& scope/*, SyntaxTree& autogenerated*/, MessageMap* attributes, ident_t sourcePath)
{
//   SNode attributes;
   SNode current = node.firstChild();
   while (current != lxNone) {
      if (current == lxScope) {
         DerivationScope rootScope(&scope, sourcePath, attributes);
...
void DerivationTransformer :: generateSyntaxTree(SyntaxWriter& writer, CompilerScope& scope, ident_t sourcePath)
{
   // list of attributes / types
   MessageMap attributes;

   writer.newNode(lxNamespace);
   writer.appendNode(lxSourcePath, sourcePath);

   // system module should be included by default
   if (!scope.module->Name().compare(STANDARD_MODULE)) {
      //importedNs.add(STANDARD_MODULE);
...
//   SyntaxTree autogeneratedTree;
   generateSyntaxTree(writer, _root, scope, /*autogeneratedTree, */&attributes, sourcePath);
