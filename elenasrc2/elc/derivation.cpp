...
{
   SNode current = node.findChild(lxOperator);
   if (current.argument == -3 && current.nextNode() == lxAttributeValue) {
      return true;
   }
...
void DerivationReader :: generateNewTemplate(SyntaxWriter& writer, SNode node, DerivationScope& scope)
{
...
void DerivationReader :: generateExpressionTree(SyntaxWriter& writer, SNode node, DerivationScope& scope, int mode)
{
   ...
   else if (current == lxObject && next == lxOperator && next.findChild(lxObject).existChild(lxAngleOperator)) {
      // check if it is new template
      generateNewTemplate(writer, node, scope);
   }
   else {
      while (current != lxNone) {
