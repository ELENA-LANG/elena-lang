...
size_t ThreadContext :: ClassVMT(size_t objectPtr)
{
   ...
   ReadProcessMemory(hProcess, (void*)(objectPtr - elPageVMTOffset64), &dump, 8, &size);
...
{
   if (address==context->context.Rip) {
      context->setTrapFlag();
   ...
   if (withStackControl) {
      context->breakpoint.stackLevel = context->context.Rbp;
   }
...
bool BreakpointContext :: processStep(ThreadContext* context, bool stepMode)
{
   ...
   if (breakpoint.hardware) {
      ...
      if (context->context.Rbp < breakpoint.stackLevel) {
         context->breakpoint.next = context->context.Rip;
         context->setTrapFlag();
         return true;
      }      
   }
...
{
   if (breakpoints.exist(context->context.Rip - 1)) {
      context->breakpoint.next = context->context.Rip - 1;
...
void Debugger :: processException(EXCEPTION_DEBUG_INFO* exception)
{
   switch (exception->ExceptionRecord.ExceptionCode) {
      case EXCEPTION_SINGLE_STEP:
         ...
         else if (current->context.Rip >= minAddress && current->context.Rip <= maxAddress) {
            processStep();
         }
         if (!trapped)
            current->setTrapFlag();
      ...
      case EXCEPTION_BREAKPOINT:
         if (breakpoints.processBreakpoint(current)) {
            current->state = steps.get(current->context.Rip);
            trapped = true;
            stepMode = false;
            current->setTrapFlag();
         }
...
bool Debugger :: proceedCheckPoint()
{
   if (started) {
      if (current->atCheckPoint) {
         current->checkFailed = (current->context.Rax == 0);
