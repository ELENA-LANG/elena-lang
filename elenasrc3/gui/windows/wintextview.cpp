   ...
   if (caret.x >= 0 && caret.y >= 0) {
      if (_caret_x == 0 || _caretChanged) {
         ...
         if (_model->highlightCurrentRow) {
            ...
            canvas.drawTransparentRectangle(Rectangle(clientRect.topLeft.x + marginWidth, lineHeight * caret.y + 1, clientRect.width() - clientRect.topLeft.x - marginWidth - 2, lineHeight), *selStyle);
         }
      }
