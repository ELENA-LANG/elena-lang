...
void SyntaxTreeBuilder :: generateTemplateOperation(SyntaxTreeWriter& writer, Scope& scope, SyntaxNode& node)
{
      ...
      arguments, parameters))
   {
      ...
      if (checkExprMode) {
         // HOTFIX : if an expression operation was used
         templateName.insert(INLINEEXPR_PREFIX, 0);
         templateRef = _moduleScope->operations.get(*templateName);
         if (_templateProcessor->importExpressionTemplate(*_moduleScope, templateRef, writer.CurrentNode(),
            arguments, parameters))
         {
            return;
...
void TemplateProssesor :: importTemplate(Type type, MemoryBase* templateSection,
   SyntaxNode target, SyntaxNode declarationNode, List<SyntaxNode>* arguments, List<SyntaxNode>* parameters)
{
   ...
   SyntaxNode writerTarget = (type == Type::InlineProperty || type == Type::InlineMethod) ? target.parentNode() : target;
      ...
      case Type::Parameterized:
      {
         ...
         while (current != SyntaxKey::None) {
            if (current == SyntaxKey::Method) {
               ...
               targetWriter.newNode(SyntaxKey::Autogenerated);
               // inject a virtual terminal to be used for error reporting
               if (declarationNode != SyntaxKey::None)
                  SyntaxTree::copyNode(targetWriter, declarationNode);
...
{
   importTemplate(Type::Class, templateSection, target, declarationNode, &parameters, nullptr);
}
...
void TemplateProssesor :: importInlineMemberTemplate(MemoryBase* templateSection,
   SyntaxNode target, bool propertyMode, List<SyntaxNode>& parameters)
{
   importTemplate(propertyMode ? Type::InlineProperty : Type::InlineMethod, templateSection, target, {}, &parameters, nullptr);
}
