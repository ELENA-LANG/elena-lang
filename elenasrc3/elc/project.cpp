...
void Project :: loadConfig(ConfigFile& config, path_t configPath, ConfigFile::Node& root)
{
   if (!root.isNotFound()) {
      ...
      loadVariables(config, root, META_VARIABLE_CATEGORY);
      loadConstants(config, root, META_CONSTANT_CATEGORY);
      ...
      loadBoolSetting(config, root, STRICT_MAPPING_PATH, ProjectOption::StrictMapping);
      loadBoolSetting(config, root, JUMP_ALIGNMENT_PATH, ProjectOption::WithJumpAlignment);
      loadBoolSetting(config, root, METHOD_OUTPUT_PATH, ProjectOption::WithMethodOutput);
