...
bool Interpreter :: evalObjArrayOp(ref_t operator_id, ArgumentsInfo& args)
{
   ...
   if (loperand.kind == ObjectKind::TypeList && roperand.kind == ObjectKind::Symbol) {
      if (operator_id == ADD_ASSIGN_OPERATOR_ID) {
         ref_t mask = roperand.mode == TargetMode::AutoSymbol ? mskAutoSymbolRef : mskSymbolRef;
...
ObjectInfo Compiler::MetaScope :: mapIdentifier(ustr_t identifier, bool referenceOne, EAttr attr)
{
   if (!referenceOne) {
      if (moduleScope->declVar.compare(identifier)) {
         ...
         ObjectInfo retVal = {};
         if (EAttrs::testAndExclude(attr, EAttr::AutoGenerated)) {
            IdentifierString weakIdentifier("'", identifier);

            NamespaceScope* ns = Scope::getScope<NamespaceScope>(*this, Scope::ScopeLevel::Namespace);
            retVal = ns->mapWeakReference(*weakIdentifier, false);
            retVal.mode = TargetMode::AutoSymbol;

            return retVal;
         }
         else if (EAttrs::testAndExclude(attr, EAttr::Superior) && parent->parent != nullptr) {
   ...
   ExpressionAttribute mode, bool withoutNewScope)
{
   ...
   else compileCode(writer, *parentCodeScope, node, false);
